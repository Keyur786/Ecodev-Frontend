            ...
            <div className="w-full max-w-4xl mx-auto bg-white rounded-lg shadow p-4">
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="grid gap-6 mb-6">
                        ...
                        <div>
                            <label htmlFor="areaNeeded" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Area Needed (ft³)</label>
                            <input type="number" id="areaNeeded" name="areaNeeded" value={formData.areaNeeded} onChange={handleInputChange}
                                className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="$15 per ft³" required />
                            <p className="text-sm text-gray-500 mt-2">Approx Cost to Store: ${storageCost}</p>
                        </div>
                        <div className="flex items-center mb-4">
                            ...
                            />
                            <label htmlFor="needVehicle" className="ml-2 text-sm font-medium text-gray-900 dark:text-white">Need Vehicle for Transport?</label>
                        </div>
                        {formData.needVehicle && (
                            <>
                                <div>
                                    <label htmlFor="vehicleType" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Vehicle Type</label>
                                    <select
                                        id="vehicleType"
                                        name="vehicleType"
                                    ...
                                    >
                                        ...
                                        {vehicleOptions.map((option, index) => (
                                            <option key={index} value={option.name}>{option.name} (${option.pricePerKm}/km)</option>
                                        ))}
                                    </select>
                                </div>
                                <div>
                                    <label htmlFor="distance" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Distance (km)</label>
                                    <input
                                        type="number"
                                        id="distance"
                                        name="distance"
                                        ...
                                        onChange={handleInputChange}
                                        className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Enter distance in km" required
                                    />
                                    <p className="text-sm text-gray-500 mt-2">Vehicle Cost: ${vehicleCost}</p>
                                </div>
                                <div>
